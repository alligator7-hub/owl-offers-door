...
<required_reading>
@/workspace/.cursor/gsd-core/references/domain-probes.md
@/workspace/.cursor/gsd-core/references/gate-prompts.md
@/workspace/.cursor/gsd-core/references/universal-anti-patterns.md
</required_reading>
...
if [ -f "/workspace/.cursor/gsd-core/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Read `@/workspace/.cursor/gsd-core/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
...
```
Apply each entry in `activeHooks` per @/workspace/.cursor/gsd-core/references/loop-hook-dispatch.md. Empty list → continue to `analyze_phase`.
</step>
...
```
Apply each entry in `activeHooks` per @/workspace/.cursor/gsd-core/references/loop-hook-dispatch.md. Empty list → continue to `confirm_creation`.
</step>
