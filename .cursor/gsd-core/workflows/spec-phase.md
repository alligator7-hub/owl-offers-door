...
ones). Reference: @/workspace/.cursor/gsd-core/references/edge-probe.md.
...
# — NOT the consuming project's git root — falling back to git toplevel / /workspace/.cursor.
...
EDGE_PROBE_JS=$(for _c in \
  ...
  "/workspace/.cursor/gsd-core/bin/lib/edge-probe.cjs" \
  "/workspace/.cursor/bin/lib/edge-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)
...
requirement list). Reference: @/workspace/.cursor/gsd-core/references/prohibition-probe.md — the
...
Use the SPEC.md template from @/workspace/.cursor/gsd-core/templates/spec.md.
