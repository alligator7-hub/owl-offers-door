 ...
 * 'Cursor' : runtime` ternary — cosmetic (log text) only, but resolves
 * to the same 'Qwen Code' / 'Cursor' values for those two runtimes.
...
function applySettingsJsonHooks(settings, opts) {
    ...
    if (hooksSurface !== 'none' && hooksSurface !== 'kimi-hooks-toml') {
        ...
        //    (#788 + #770 + #2092) ────────────────────────────────────────────────
        // Cursor (since #770) and Qwen Code (since #788) both support the
        ...
        // Cursor fires FileChanged when a watched file changes on disk.  Wire
        ...
        // (Cursor matches by filename, not full path).  The hook exits silently
        // when the changed file is not the gsd config.
        //
        // Scoped to Cursor only: Qwen Code's FileChanged support is not yet
        ...
        if (extendedEvents.includes('FileChanged')) {
            ...
            if (!alreadyHasConfigReload && node_fs_1.default.existsSync(configReloadFile) && configReloadCommand) {
                ...
                console.log(`  ${green}✓${reset} Configured FileChanged config-reload hook (Cursor)`);
            }
