...
// Tool name mapping from Cursor to OpenCode
...
/**
 * Convert a Cursor tool name to OpenCode format
 ...
 * by Cursor as the skill identity. Emits the hyphen form (gsd-<cmd>)
 * so Cursor autocomplete shows the canonical invocation form, not the
 ...
 * Cursor is the native format, so minimal conversion needed —
...
function convertClaudeCommandToClaudeSkill(content, skillName, runtime = null, cmdNames = null) {
    ...
    // `name:` Cursor (and Qwen/Hermes) register under (#2808). `cmdNames`
 ...
 *
 * WHY this can't stay a static `@`-include (even a relative one): Cursor
 * documents relative `@`-paths as resolving against the file *containing* the
 * import, which for a global skill is `~/.cursor/skills/gsd-<stem>/` — not
 ...
 * Path mappings depend on install mode:
 *   Global: ~/.cursor/skills/ → ~/.gemini/config/skills/ (#3738),
...
function convertClaudeToCursorMarkdown(content) {
    ...
    // Remove Cursor-specific bug workarounds before brand replacement
    converted = converted.replace(/\*\*Known Cursor bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Cursor" brand references with "Cursor" — #2284(b): skips
...
// without this would have silently regressed live installs: `Cursor`
...
function convertClaudeToWindsurfMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Cursor bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Cursor" brand references with "Windsurf" — #2284(b): skips
    // <runtime_compatibility> comparison-table content (protected region).
...
function convertClaudeToAugmentMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Cursor bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Cursor" brand references with "Augment" — #2284(b): skips
    // <runtime_compatibility> comparison-table content (protected region).
...
function convertClaudeToTraeMarkdown(content) {
    ...
    // unprefixed `./.cursor/rules/` form the very next pattern handles differently
...
function convertClaudeToCodebuddyMarkdown(content) {
    ...
    // CodeBuddy uses the same tool names as Cursor (Bash, Edit, Read, Write, etc.)
...
/**
 * Convert a Cursor slash-command (.md) to a CodeBuddy slash-command (.md).
 *
...
function convertClaudeToCliineMarkdown(content) {
    ...
    // Cline uses the same tool names as Cursor — no tool name conversion needed
...
function convertClaudeToCodexMarkdown(content) {
    ...
    // "check `.cursor/skills/`" where there is no ~/, $HOME/, or ./ anchor.
...
GSD workflows use \`AskUserQuestion\` (Cursor syntax). Translate to Codex \`request_user_input\`:
...
GSD workflows use \`Task(...)\` (Cursor syntax). Translate to Codex collaboration tools:
...
function neutralizeAgentReferences(content, instructionFile) {
    ...
    // Negative lookahead avoids: Cursor, Claude Opus/Sonnet/Haiku, Claude native, Claude-based
    c = c.replace(/\bClaude(?! Code| Opus| Sonnet| Haiku| native| based|-)\b(?!\.md)/g, 'the agent');
    // Replace .cursor/rules/ with runtime-appropriate instruction file
    if (instructionFile) {
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false, modelOverride = null, variant = null } = {}) {
    ...
    for (const line of lines) {
        ...
        // Strip model: field — OpenCode doesn't support Cursor model aliases
...
function convertClaudeToKiloFrontmatter(content, { isAgent = false, modelOverride = null } = {}) {
    ...
    convertedContent = replaceRelativePathReference(convertedContent, '.cursor/skills/', '.kilo/skills/');
    ...
    for (const line of lines) {
        ...
        // Strip model: field — Kilo doesn't support Cursor model aliases
...
// Copilot tool name mapping — Cursor tools to GitHub Copilot tools
...
// Tool name mapping from Cursor to Gemini CLI
...
/**
 * Convert a Cursor tool name to Gemini CLI format
...
/**
 * Convert a Cursor tool name to GitHub Copilot format.
...
/**
 * Convert Cursor agent markdown to Cursor agent format.
...
/**
 * Convert Cursor agent markdown to Windsurf agent format.
...
/**
 * Convert Cursor agent markdown to Augment agent format.
 ...
 *
 * `color` IS preserved: Qwen's docs list `color` under "Cursor
 ...
 *
 * Body: preserved verbatim after the qwen branding rewrite (.cursor/rules/ /
 * Cursor / .claude/ literal-substring values — descriptor-driven via
...
function convertClaudeAgentToQwenAgent(content) {
    ...
    let converted = content;
    if (_b['.cursor/rules/'])
        converted = converted.replace(/CLAUDE\.md/g, _b['.cursor/rules/']);
    // #2284(b): skips <runtime_compatibility> comparison-table content (protected region).
    if (_b['Cursor'])
        converted = applyClaudeCodeBrandSwap(converted, _b['Cursor']);
    if (_b['.claude/'])
...
/**
 * Convert a Cursor agent .md for ZCode (#3384).
 *
 ...
 * not connected") whenever it is not connected, whereas Cursor treats the
 ...
 *   .cursor/rules/    -> brandingRewrites['.cursor/rules/']
 *   Cursor  -> brandingRewrites['Cursor']  (word-boundary, \bClaude Code\b)
...
function applyAgentBrandingRewrites(content, runtime) {
    ...
    if (_b['.cursor/rules/'])
        converted = converted.replace(/CLAUDE\.md/g, _b['.cursor/rules/']);
    if (_b['Cursor'])
        converted = converted.replace(/\bClaude Code\b/g, _b['Cursor']);
    if (_b['.claude/'])
        converted = converted.replace(/\.claude\//g, _b['.claude/']);
    return converted;
}
...
/**
 * Convert Cursor agent markdown to Codex agent format.
 ...
 * non-Claude runtime, because GSD's worktree isolation was Cursor's
 ...
 * not expand inside double-quoted shell strings (#1284). But Cursor's
 ...
 * automated test can verify *resolution* inside a live Cursor session
 ...
 * `~` form Cursor documents as expanding. A single pathPrefix string
 ...
 * Cursor documents `@`-references as valid "anywhere in your
 * .cursor/rules/" (e.g. `See @README for project overview`), so anchoring to
...
function _applyRuntimeRewrites(content, runtime, pathPrefix, isGlobal = false, attribution = undefined) {
    ...
    switch (runtime) {
        ...
        // brand VALUES (.cursor/rules//Cursor/.claude/ replacements) now read from
        ...
        case 'qwen': {
            ...
            // rewrites below) instead of throwing on `_b['.cursor/rules/']`.
            const _b = _hostBehaviors(runtime).brandingRewrites;
            if (_b) {
                content = content.replace(/CLAUDE\.md/g, _b['.cursor/rules/']);
                // #2284(b): skips <runtime_compatibility> comparison-table content (protected region).
                content = applyClaudeCodeBrandSwap(content, _b['Cursor']);
            }
            content = content.replace(/~\/\.claude\//g, pathPrefix);
            content = content.replace(/\$HOME\/\.claude\//g, pathPrefix);
...
function applyAgentFrontmatterExtensions(content, { runtime, agentName, targetDir }) {
    ...
    // #3533 (10d): 'inherit' means the effort: key must NOT exist — Cursor
